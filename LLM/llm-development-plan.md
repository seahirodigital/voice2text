...
- Voice2Text local control folder: `%USERPROFILE%\OneDrive\開発\Voice2Text\LLM`
- Ollama model storage target: `%LOCALAPPDATA%\Voice2Text\ollama\models`
- Voice2Text LLM logs target: `%LOCALAPPDATA%\Voice2Text\ollama\logs`
- Voice2Text LLM scripts/prompts target: `%USERPROFILE%\OneDrive\開発\Voice2Text\LLM`
...
- `gemma4:e2b` and `gemma4:e4b` are pulled into `%LOCALAPPDATA%\Voice2Text\ollama\models`.
...
1. Create the local LLM folder layout.
   - `%LOCALAPPDATA%\Voice2Text\ollama\models`
   - `%LOCALAPPDATA%\Voice2Text\ollama\logs`
   - `LLM/prompts`
   - `LLM/scripts`
2. Repair `config.json` so the backend can load settings.
3. Install Ollama for Windows.
4. Set `OLLAMA_MODELS` to `%LOCALAPPDATA%\Voice2Text\ollama\models`.
...
- Do not place Ollama models or logs under OneDrive. Keep heavy generated assets in `%LOCALAPPDATA%\Voice2Text\...`.
