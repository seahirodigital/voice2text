...
- Ollama models: `%LOCALAPPDATA%\Voice2Text\ollama\models`
- Ollama logs: `%LOCALAPPDATA%\Voice2Text\ollama\logs`

## 絶対制限事項

- OneDrive 配下にはソースコード、設定ファイル、スクリプト、軽量なテキストだけを置きます。
- `LLM\models`、`LLM\logs`、仮想環境、`node_modules`、録音データ、各種モデルキャッシュのような重量データを OneDrive 配下へ置いてはいけません。
- Ollama のモデルとログは必ず `%LOCALAPPDATA%\Voice2Text\ollama\...` に置きます。Moonshine / Faster Whisper / 録音データも `%LOCALAPPDATA%\Voice2Text\...` 配下を使います。
- `C:\Users\<user>\OneDrive\開発\Voice2Text\LLM\models` や同等の OneDrive 配下に重い実体データが残っていたら、起動前にローカルへ移動します。
...
Groq を使う場合の API キーは `config.json` に直接書かず、`LLM/.env/.env` などの Git 管理外ファイルに `GROQ_API_KEY=...` として置きます。
...
1. Moonshine `base` で録音ファイルを一括文字起こしします。
...
- `LLM\scripts\start-ollama.ps1` で `OLLAMA_MODELS` と `OLLAMA_HOST` を明示し、保存先を `%LOCALAPPDATA%\Voice2Text\ollama` に固定して起動します。
...
- `LLM\models` と `LLM\logs` を OneDrive 配下の実データ置き場として使ってはいけません。生成されていた場合は `%LOCALAPPDATA%\Voice2Text\ollama\...` へ戻してください。
